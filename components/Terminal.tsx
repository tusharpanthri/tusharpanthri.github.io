...
const Terminal = () => {
  ...
  const [history, setHistory] = useState<Line[]>([{ type: 'output', text: "Welcome. Type 'help' to get started." }]);
  ...
  return (
    <div className="no-print fixed inset-0 z-[300] flex items-end justify-center bg-near-black/40" onClick={() => setTerminalOpen(false)}>
      <div className="w-full max-w-[1080px] border-2 border-b-0 border-near-black overflow-hidden" style={{ background: '#11100D', animation: 'termRise 0.25s ease-out' }} onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center gap-2 px-4 py-3 border-b-2 border-[#2a2820]">
          <span className="w-2 h-2 bg-accent-red"></span><span className="w-2 h-2 bg-accent-yellow"></span><span className="w-2 h-2 bg-cream"></span>
          <span className="ml-3 font-mono text-xs text-[#6E675A] uppercase tracking-widest">{resume.name.toLowerCase().replace(/\s+/g, '-')}@portfolio: ~</span>
          <button onClick={() => setTerminalOpen(false)} className="ml-auto font-mono text-xs text-[#6E675A] hover:text-cream transition-colors" aria-label="Close terminal">esc</button>
        </div>
        <div ref={bodyRef} className="h-80 md:h-96 overflow-y-auto px-4 py-4 font-mono text-sm">
          {history.map((line, i) => <div key={i} className={line.type === 'input' ? 'text-cream' : 'text-[#3ddc5b] whitespace-pre-wrap'}>{line.type === 'input' ? `${PROMPT} ${line.text}` : line.text}</div>)}
          <form onSubmit={handleSubmit} className="flex items-center gap-2 text-[#3ddc5b] mt-1">
            <span>{PROMPT}</span>
            <input ref={inputRef} aria-label="Terminal command" value={input} onChange={(e) => setInput(e.target.value)} className="flex-1 bg-transparent outline-none text-cream caret-transparent" autoComplete="off" autoCapitalize="off" spellCheck={false} />
