...
const GitHubStats = () => {
  ...
  return (
    <section className="py-12 border-t-4 border-near-black">
      <div className="max-w-7xl mx-auto px-6 md:px-10">
        ...
        <div className="grid gap-6" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))' }}>
          {cards.map((card) => (
            <div
              key={card.label}
              className="bg-white border-4 border-near-black p-6 shadow-[6px_6px_0_#11100D]"
            >
              <div className="text-4xl md:text-5xl font-black tabular-nums">{card.value}</div>
              <div className="font-mono text-xs font-bold uppercase tracking-widest text-muted mt-2">
                {card.label}
              </div>
            </div>
          ))}
